...
const food = {
   ...
   */
  isOnPoint(point) {
    return this.getCoordinates().x === point.x && this.getCoordinates().y === point.y;
  },
};
...
const game = {
   ...
   */
  canMakeStep() {
    let { x, y } = this.snake.getNextStepHeadPoint();
    let direction = this.snake.direction;
    let snakeBody = this.snake.getBody();
    return !((x === 20 && direction === 'left') ||
        (x === 0 && direction === 'right') ||
        (y === 20 && direction === 'up') ||
        (y === 0 && direction === 'down') ||
        (snakeBody.some(bodyCell => bodyCell.x === x && bodyCell.y === y)));
  },
};
